...
export default async function AdminHistoriquePage({ searchParams }: { searchParams: Promise<{ action?: string }> }) {
  await requireAdmin();
  ...
  return (
    <section className="admin-panel">
      ...
      >
        ...
        <a
          className="button"
          href={`/api/historique/export${action ? `?action=${encodeURIComponent(action)}` : ""}`}
          download
        >
          Exporter CSV
        </a>
      ...
      <div className="filter-bar" style={{ justifyContent: "flex-start" }}>
        <Link href="/admin/historique" className={`filter-chip${!action ? " active" : ""}`}>
          Tout
        </Link>
        {actions.map((a) => (
          <Link
            key={a}
            href={`/admin/historique?action=${encodeURIComponent(a)}`}
